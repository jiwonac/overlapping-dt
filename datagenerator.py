...
def data_generator(num_sources, distribution, costs, num_groups, group_dists):
	...
	"""
	data_sources = [[] for source in range(num_sources + 1)]
		...
		for source in key:
			...
			if source > 0:
				for i in range(value):
					...
					data_sources[source].append(data_point)
	...
	for source in range(1, num_sources + 1):
		...
		for group in range(num_groups):
			for j in range(group_dist[group]):
				data_sources[source][i]["group"] = group
				i += 1
	return {"sources": data_sources, "distribution": distribution, 
		"num_sources": num_sources, "num_groups": num_groups, "costs": costs, "group_dists": group_dists}
